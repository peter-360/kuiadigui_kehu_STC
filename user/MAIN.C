...
void feed_dog() {
  // WDT_CONTR = (D_EN_WDT + D_CLR_WDT + D_WDT_SCALE_256);
  WDT_CONTR |= 0x10; //清看门狗,否则系统复位
}
void lkonoff_led_dog() {
  feed_dog();
  // LED1 = !LED1;
}
...
void data_parse(uint8_t *RX1_Buffer_0, uint16_t Uart1_Rx_0) {
  ...
  DSW_T_5 = ~DSW_5;
  DSW_T_6 = ~DSW_6;
  board_addr = DSW_T_1 | (DSW_T_2 << 1) | (DSW_T_3 << 2) | (DSW_T_4 << 3) |
               (DSW_T_5 << 4) | (DSW_T_6 << 5);
  ...
  if (board_addr == Uart1_Buffer_T[1]) // todo
  {
    if (5 == Uart1_Rx_T) {
      ...
      if (bcc_temp == m_data.bcc) {
        ...
        case 0x8A:
          ...
          if (m_data.gu_ding == 0x11) // process
          {
            ...
            case 1:
              ...
              // LED1 =1;
              gpio_level = GI_1;
              // SEGGER_RTT_printf(0, "%d:gpio_level =
        ...
        case 0x80:
          if (m_data.gu_ding == 0x33) // process
          {
            ...
            {
              //								debug_uart_send_data1((GI_T_1)
              //| (GI_T_2<<1));
              grp_level_1 = GI_T_1 | (GI_T_2 << 1) | (GI_T_3 << 2) |
                            (GI_T_4 << 3) | (GI_T_5 << 4) | (GI_T_6 << 5) |
                            (GI_T_7 << 6) | (GI_T_8 << 7);
      ...
      if (bcc_temp == m_data.bcc) {
        ...
        case 0xf2: //--------2.7--------- hardware version and dan/shuang xian
                   //type  add
          // SEGGER_RTT_printf(0, "ok,m_data.opcode=%02x\n",m_data.opcode);
          memset(tx_Buffer, 0, 256);
          memcpy(tx_Buffer, "star", 4);
...
bool spear_uart_process_data(uint8_t byt) {
  ...
  case UART_OPCODE:
    ...
    if ((0x8A == g_data.opcode) || (0x80 == g_data.opcode)) {
      g_data.len = 3 + 1;
    } else if ((0x90 == g_data.opcode) || (0x91 == g_data.opcode) ||
               (0xf1 == g_data.opcode) || (0xf2 == g_data.opcode)) {
      g_data.len = 1 + 1;
    } else {
      g_data.len = 1 + 1;
    }
...
uint8 QueueIn(struct FifoQueue *Queue, ElemType sdat) {
  if ((Queue->front == Queue->rear) && (Queue->count == QueueSize)) { // full
    return QueueFull;
  } else { // in
...
uint8 QueueOut(struct FifoQueue *Queue, ElemType *sdat) {
  if ((Queue->front == Queue->rear) && (Queue->count == 0)) { // empty
    return QueueEmpty;
  } else { // out
...
void main(void) {
  ...
  while (1) {
    ...
    tick_times++;
    if (0 == red_flag) {
      // if(tick_times%120000==0)//
      if (tick_times % 40000 == 0) // 1000     1.15s
      {
        tick_times = 0;
        red_flag = 1;
        LED1 = 0; // on
